...
BOOL CMainFrame::OnCreateClient(LPCREATESTRUCT lpcs, CCreateContext* pContext)
{
	...
	m_wndSplitter.CreateStatic(this, 1, 2);  //1 - строка, 2 - столбец

	m_wndSplitter.CreateView(0, 0, RUNTIME_CLASS(CMyTreeView), CSize(400, 0), pContext);
	m_wndSplitter.CreateView(0, 1, RUNTIME_CLASS(CSINView), CSize(0, 0), pContext);
	
	SetActiveView((CView*)m_wndSplitter.GetPane(0, 1));

	CSINDoc* pDoc = (CSINDoc*)GetActiveDocument();
